...
export default function SiteBuilderAdmin() {
  ...
  return <>
    <div className="admin-page-heading-v5"><div><span>EDITOR DO SITE</span><h1>Monte a página inicial</h1><p>Ative, reordene e edite cada bloco. A ordem abaixo é a ordem publicada.</p></div><div><a className="admin-button-v5 secondary" href="/" target="_blank"><Icon name="eye" />Ver site</a><button className="admin-button-v5" onClick={() => void addCustom()}><Icon name="plus" />Novo bloco</button></div></div>
    {error ? <div className="admin-alert-v5 error">{error}</div> : null}{message ? <div className="admin-alert-v5 success">{message}</div> : null}
    <div className="builder-layout-v5">
      <aside className="builder-list-v5">
      ...
      <section className="builder-editor-v5">
        {selected ? <>
          ...
          <div className="builder-preview-v5"><small>PRÉVIA DO BLOCO</small><div className={`builder-preview-card ${selected.section_type}`}><span>{selected.eyebrow}</span><h3>{selected.title || labels[selected.section_type]}</h3><p>{selected.subtitle}</p>{selected.section_type === "categories" ? <div className="mini-stories"><i /><i /><i /><i /></div> : null}{["newest","trending","video_products","catalog"].includes(selected.section_type) ? <div className="mini-products"><i /><i /><i /><i /></div> : null}</div></div>
          <div className="builder-save-row"><small>As alterações entram no ar ao salvar.</small><div>{selected.section_type === "custom_text" ? <button className="admin-button-v5 danger" onClick={() => void deleteSelected()}><Icon name="trash" />Excluir</button> : null}<button className="admin-button-v5 secondary" onClick={() => void duplicateSelected()}><Icon name="copy" />Duplicar</button><button className="admin-button-v5" onClick={() => void saveSection()} disabled={saving}><Icon name="save" />{saving ? "Salvando..." : "Salvar bloco"}</button></div></div>
        </> : <div className="empty-v5">Selecione um bloco para editar.</div>}
      </section>
    </div>
  </>;
}
