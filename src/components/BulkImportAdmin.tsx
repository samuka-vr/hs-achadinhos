...
function validateItem(item: Omit<ParsedItem, "errors">) {
  ...
  if (!item.categoryName) errors.push("Categoria não identificada");
  if (item.categoryName.length > 80) errors.push("Categoria com mais de 80 caracteres");
...
function parseProducts(text: string): ParsedItem[] {
  ...
  return matches.map((match, index) => {
    ...
    const categoryName = extractField(body, /Categoria:/, ["Descrição", "Descricao", "Valor", "Preço", "Preco", "Link"]);
...
export default function BulkImportAdmin() {
  ...
  async function ensureCategory(name: string, categoryMap: Map<string, ExistingCategory>) {
    ...
    if (!autoCreateCategories) throw new Error(`A categoria “${name}” ainda não existe.`);

    const supabase = getBrowserSupabase();
    if (!supabase) throw new Error("Supabase não configurado.");
    const usedSlugs = new Set(Array.from(categoryMap.values()).map((category) => category.slug));
    const baseSlug = slugify(name) || `categoria-${Date.now()}`;
    let categorySlug = baseSlug;
    let suffix = 2;
    while (usedSlugs.has(categorySlug)) categorySlug = `${baseSlug}-${suffix++}`;
  ...
  return <>
    ...
    {step === "paste" ? <section className="admin-card-v5 bulk-paste-card">
      <div className="bulk-card-title"><div><small>ENTRADA</small><h2>Cole os produtos</h2><p>O importador reconhece número, nome, categoria, descrição, valor e link.</p></div><button type="button" className="text-button" onClick={() => setRawText(EXAMPLE)}>Usar exemplo</button></div>
      <textarea
    ...
    {step === "review" ? <>
      ...
      <section className="admin-card-v5 bulk-options-card">
        ...
        <label className="bulk-option"><input type="checkbox" checked={autoCreateCategories} onChange={(event) => setAutoCreateCategories(event.target.checked)}/><span><strong>Criar categorias automaticamente</strong><small>Novas categorias serão criadas usando exatamente os nomes da lista.</small></span></label>
        ...
        {missingCategories.length ? <div className="bulk-new-categories"><span>Serão criadas:</span>{missingCategories.map((category) => <b key={category}>{category}</b>)}</div> : null}
      </section>
      ...
      <section className="bulk-review-list">
        ...
        {items.map((item) => <article className={`bulk-review-item ${!item.enabled ? "disabled" : ""} ${item.errors.length ? "has-error" : ""}`} key={item.tempId}>
          ...
          <div className="bulk-review-fields">
            ...
            <label><span>Categoria</span><input value={item.categoryName} onChange={(event) => updateItem(item.tempId, { categoryName: event.target.value })}/></label>
