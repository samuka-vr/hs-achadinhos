...
export default function ControlCenterAdmin() {
  ...
  async function exportCsv() {
    ...
    if (loadError) setError(loadError.message);
      ...
      const rows = ((data ?? []) as ProductCsvRow[]).map((item) => [item.product_code, item.name, item.categories?.name, item.current_price, item.old_price, item.affiliate_url, item.image_url, item.is_active ? "sim" : "nao"]);
  ...
  return <>
    <div className="admin-page-heading-ui control-heading-ops">
      ...
      <div><Link className="admin-button-ui secondary" href="/" target="_blank"><Icon name="eye" />Ver site</Link><Link className="admin-button-ui" href="/admin/produtos/importar"><Icon name="plus" />Importar produtos</Link></div>
    </div>

    {error ? <div className="admin-alert-ui error">{error}</div> : null}
    {message ? <div className="admin-alert-ui success">{message}</div> : null}

    <section className="control-summary-ops">
    ...
    <nav className="control-tabs-ops">{tabs.map(([key, label, description]) => <button key={key} className={tab === key ? "active" : ""} onClick={() => setTab(key)}><strong>{label}</strong><small>{description}</small></button>)}</nav>

    {tab === "site" ? <form onSubmit={saveQuickSettings} className="control-grid-ops">
      <section className="admin-card-ui control-panel-ops">
        <div className="admin-card-head-ui"><div><small>PUBLICAÇÃO</small><h2>Status do site</h2><p>Pause o site público sem bloquear o painel.</p></div><label className="switch-ui"><input type="checkbox" checked={settings.maintenance_mode} onChange={(event) => update("maintenance_mode", event.target.checked)} /><span /><b>{settings.maintenance_mode ? "Manutenção" : "Online"}</b></label></div>
        <div className="admin-form-grid-ui"><label className="full"><span>Título da manutenção</span><input value={settings.maintenance_title} onChange={(event) => update("maintenance_title", event.target.value)} /></label><label className="full"><span>Mensagem</span><textarea rows={3} value={settings.maintenance_message} onChange={(event) => update("maintenance_message", event.target.value)} /></label></div>
      </section>

      <section className="admin-card-ui control-panel-ops">
        <div className="admin-card-head-ui"><div><small>AVISO</small><h2>Faixa do topo</h2><p>Mensagem rápida para lançamentos e campanhas.</p></div><label className="switch-ui"><input type="checkbox" checked={settings.announcement_enabled} onChange={(event) => update("announcement_enabled", event.target.checked)} /><span /><b>{settings.announcement_enabled ? "Visível" : "Oculta"}</b></label></div>
        <div className="admin-form-grid-ui"><label className="full"><span>Texto</span><input value={settings.announcement_text} onChange={(event) => update("announcement_text", event.target.value)} /></label><label className="full"><span>Destino</span><input value={settings.announcement_url} onChange={(event) => update("announcement_url", event.target.value)} placeholder="#produtos" /></label></div>
      </section>

      <section className="admin-card-ui control-panel-ops control-wide-ops">
        <div className="admin-card-head-ui"><div><small>FOOTER</small><h2>Redes sociais</h2><p>Somente os campos preenchidos aparecem no rodapé.</p></div><Link href="/admin/navegacao">Editor completo <Icon name="arrow" size={15} /></Link></div>
        <div className="admin-form-grid-ui"><label><span>Instagram</span><input type="url" value={settings.instagram} onChange={(event) => update("instagram", event.target.value)} placeholder="https://instagram.com/..." /></label><label><span>TikTok</span><input type="url" value={settings.tiktok} onChange={(event) => update("tiktok", event.target.value)} placeholder="https://tiktok.com/@..." /></label><label><span>Vitrine Shopee</span><input type="url" value={settings.shopee_showcase} onChange={(event) => update("shopee_showcase", event.target.value)} placeholder="https://shopee.com.br/..." /></label><label><span>WhatsApp</span><input type="url" value={settings.whatsapp} onChange={(event) => update("whatsapp", event.target.value)} placeholder="https://wa.me/..." /></label><label><span>Título do footer</span><input value={settings.footer_social_title} onChange={(event) => update("footer_social_title", event.target.value)} /></label><label><span>Texto curto</span><input value={settings.footer_social_subtitle} onChange={(event) => update("footer_social_subtitle", event.target.value)} /></label></div>
      </section>

      <section className="admin-card-ui control-panel-ops control-wide-ops">
        <div className="admin-card-head-ui"><div><small>ESTADOS VAZIOS</small><h2>Mensagens do catálogo</h2><p>Textos exibidos quando nenhuma busca ou filtro encontra produtos.</p></div></div>
      ...
      <div className="control-save-ops"><button className="admin-button-ui" disabled={saving}><Icon name="save" />{saving ? "Publicando..." : "Salvar controles"}</button></div>
    </form> : null}

    {tab === "catalog" ? <div className="control-catalog-ops">
      <section className="admin-card-ui catalog-health-ops">
        <div className="admin-card-head-ui"><div><small>DIAGNÓSTICO</small><h2>Saúde do catálogo</h2><p>Problemas que podem impedir produtos de aparecerem corretamente.</p></div><Link href="/admin/produtos">Abrir produtos <Icon name="arrow" size={15} /></Link></div>
        <div className="health-grid-ops">
      ...
      <section className="operation-grid-ops">
        <article className="admin-card-ui"><span className="operation-icon-ops"><Icon name="categories" /></span><h3>Restaurar categorias oficiais</h3><p>Cria ou corrige as sete categorias principais sem apagar produtos.</p><button className="admin-button-ui secondary" disabled={Boolean(busy)} onClick={() => void runRpc("admin_restore_main_categories", "Categorias oficiais restauradas.")}><Icon name="categories" />Restaurar</button></article>
        <article className="admin-card-ui"><span className="operation-icon-ops"><Icon name="check" /></span><h3>Publicar produtos prontos</h3><p>Publica todos os rascunhos que já possuem imagem e link.</p><button className="admin-button-ui secondary" disabled={Boolean(busy) || !health.readyDrafts} onClick={() => void runRpc("admin_publish_ready_drafts", "Produtos prontos publicados.")}><Icon name="check" />Publicar {health.readyDrafts}</button></article>
        <article className="admin-card-ui"><span className="operation-icon-ops"><Icon name="products" /></span><h3>Mover tudo para rascunho</h3><p>Oculta todos os produtos do site sem excluí-los.</p><button className="admin-button-ui secondary" disabled={Boolean(busy) || !health.published} onClick={() => { setConfirmAction("draft-all"); setConfirmation(""); }}><Icon name="eye" />Ocultar catálogo</button></article>
        <article className="admin-card-ui danger-zone-ops"><span className="operation-icon-ops"><Icon name="trash" /></span><h3>Excluir produtos sem imagem</h3><p>Remove permanentemente todos os cadastros que ainda não possuem capa.</p><button className="admin-button-ui danger" disabled={Boolean(busy) || !health.noImage} onClick={() => { setConfirmAction("delete-no-image"); setConfirmation(""); }}><Icon name="trash" />Excluir {health.noImage}</button></article>
      </section>

      <section className="admin-card-ui reset-catalog-ops"><div><small>ZONA DE REDEFINIÇÃO</small><h2>Começar o catálogo novamente</h2><p>Apaga todos os produtos e categorias atuais e recria somente as sete categorias oficiais. Configurações, páginas, redes e aparência permanecem.</p></div><button className="admin-button-ui danger" onClick={() => { setConfirmAction("reset"); setConfirmation(""); }}><Icon name="trash" />Redefinir catálogo</button></section>
    </div> : null}

    {tab === "backup" ? <div className="control-backup-ops">
      <section className="operation-grid-ops backup-actions-ops">
        <article className="admin-card-ui"><span className="operation-icon-ops"><Icon name="code" /></span><h3>Backup completo em JSON</h3><p>Produtos, categorias, configurações, páginas, banners e menus.</p><button className="admin-button-ui" disabled={Boolean(busy)} onClick={() => void exportBackup()}><Icon name="external" />{busy === "export" ? "Preparando..." : "Baixar JSON"}</button></article>
        <article className="admin-card-ui"><span className="operation-icon-ops"><Icon name="products" /></span><h3>Planilha de produtos</h3><p>Baixe o catálogo em CSV para abrir no Excel ou Google Planilhas.</p><button className="admin-button-ui secondary" disabled={Boolean(busy)} onClick={() => void exportCsv()}><Icon name="external" />{busy === "csv" ? "Preparando..." : "Baixar CSV"}</button></article>
      </section>
      <section className="admin-card-ui snapshot-create-ops"><div><small>CÓPIA NO PAINEL</small><h2>Salvar ponto de segurança</h2><p>Guarda uma cópia dos dados atuais dentro do Supabase para consulta e download posterior.</p></div><div><input value={snapshotName} onChange={(event) => setSnapshotName(event.target.value)} placeholder="Ex.: Antes de alterar a página inicial" /><button className="admin-button-ui" disabled={Boolean(busy)} onClick={() => void saveSnapshot()}><Icon name="save" />{busy === "snapshot" ? "Salvando..." : "Salvar cópia"}</button></div></section>
      <section className="admin-card-ui snapshot-list-ops"><div className="admin-card-head-ui"><div><small>HISTÓRICO</small><h2>Cópias salvas</h2></div><span className="studio-count-badge">{snapshots.length}</span></div>{snapshots.map((snapshot) => <article key={snapshot.id}><div><strong>{snapshot.name}</strong><small>{new Date(snapshot.created_at).toLocaleString("pt-BR")}</small></div><button onClick={() => void downloadSnapshot(snapshot)}><Icon name="external" size={17} />Baixar</button><button className="danger" onClick={() => void deleteSnapshot(snapshot)}><Icon name="trash" size={17} /></button></article>)}{!snapshots.length ? <div className="studio-empty-mini">Nenhuma cópia salva ainda.</div> : null}</section>
    </div> : null}

    {tab === "notes" ? <div className="control-notes-ops">
      <section className="admin-card-ui"><div className="admin-card-head-ui"><div><small>LEMBRETES</small><h2>Anotações internas</h2><p>Este texto aparece somente para administradores.</p></div></div><textarea rows={12} value={settings.admin_notes} onChange={(event) => update("admin_notes", event.target.value)} placeholder="Ex.: atualizar os preços na sexta-feira..." /><button className="admin-button-ui" disabled={saving} onClick={() => void saveQuickSettings()}><Icon name="save" />Salvar notas</button></section>
      <section className="admin-card-ui activity-preview-ops"><div className="admin-card-head-ui"><div><small>ATIVIDADE RECENTE</small><h2>Últimas alterações</h2></div><Link href="/admin/atividade">Ver histórico</Link></div>{activity.map((item) => <article key={item.id}><span><Icon name={item.action === "delete" ? "trash" : item.action === "insert" ? "plus" : "edit"} size={16} /></span><div><strong>{item.summary || item.entity_type}</strong><small>{item.action} · {item.entity_type} · {new Date(item.created_at).toLocaleString("pt-BR")}</small></div></article>)}{!activity.length ? <div className="studio-empty-mini">Nenhuma alteração registrada.</div> : null}</section>
    </div> : null}

    {confirmAction ? <div className="control-modal-backdrop-ops" onMouseDown={() => setConfirmAction(null)}><div className="control-modal-ops" onMouseDown={(event) => event.stopPropagation()}><span className="control-modal-icon-ops"><Icon name={confirmAction === "draft-all" ? "eye" : "trash"} /></span><h2>{confirmAction === "reset" ? "Redefinir todo o catálogo?" : confirmAction === "delete-no-image" ? "Excluir produtos sem imagem?" : "Mover tudo para rascunho?"}</h2><p>{confirmAction === "reset" ? "Todos os produtos e categorias atuais serão apagados. Esta ação não pode ser desfeita sem um backup." : confirmAction === "delete-no-image" ? `${health.noImage} produto(s) serão excluídos permanentemente.` : `${health.published} produto(s) deixarão de aparecer no site.`}</p><label><span>Digite CONFIRMAR</span><input autoFocus value={confirmation} onChange={(event) => setConfirmation(event.target.value.toUpperCase())} /></label><div><button className="admin-button-ui secondary" onClick={() => setConfirmAction(null)}>Cancelar</button><button className="admin-button-ui danger" disabled={confirmation !== "CONFIRMAR" || Boolean(busy)} onClick={() => void runRpc(confirmAction === "reset" ? "admin_reset_catalog" : confirmAction === "delete-no-image" ? "admin_delete_products_without_images" : "admin_move_all_to_draft", confirmAction === "reset" ? "Catálogo redefinido." : confirmAction === "delete-no-image" ? "Produtos sem imagem excluídos." : "Produtos movidos para rascunho.")}><Icon name="check" />Confirmar</button></div></div></div> : null}
  </>;
}
