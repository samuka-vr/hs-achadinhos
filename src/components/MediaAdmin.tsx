...
export default function MediaAdmin() {
  ...
  async function upload(selected?: File) { if (!selected) return; setUploading(true); setError(""); const supabase = getBrowserSupabase(); if (!supabase) return; try { const ext = selected.name.split(".").pop() || "jpg"; const base = selected.name.replace(/\.[^.]+$/, "").replace(/[^a-z0-9-]+/gi,"-").toLowerCase(); const path = `library/${base}-${Date.now()}.${ext}`; const result = await supabase.storage.from("site-assets").upload(path, selected, { contentType: selected.type }); if (result.error) throw result.error; setMessage("Imagem enviada para a biblioteca."); await load(); } catch (caught) { setError(caught instanceof Error ? caught.message : "Erro no upload."); } finally { setUploading(false); } }
