...
export default function MediaAdmin() {
  ...
  return <><div className="admin-page-heading-ui"><div><span>ARQUIVOS</span><h1>Biblioteca de mídia</h1><p>Reutilize imagens já enviadas e copie a URL quando precisar.</p></div><label className="admin-button-ui"><Icon name="image" />{uploading ? "Enviando..." : "Enviar imagem"}<input type="file" accept="image/*" hidden onChange={(e) => void upload(e.target.files?.[0])} /></label></div>{error ? <div className="admin-alert-ui error">{error}</div> : null}{message ? <div className="admin-alert-ui success">{message}</div> : null}<div className="media-toolbar-ui"><div><Icon name="search" /><input placeholder="Buscar arquivo" value={search} onChange={(e) => setSearch(e.target.value)} /></div><select value={bucket} onChange={(e) => setBucket(e.target.value as typeof bucket)}><option value="all">Todos os arquivos</option><option value="site-assets">Marca, categorias e banners</option><option value="product-images">Produtos</option></select></div><div className="media-grid-ui">{visible.map((item) => <article key={`${item.bucket}-${item.name}`}><div><img src={item.url} alt="" loading="lazy" /></div><strong>{item.name.split("/").pop()}</strong><small>{item.bucket === "site-assets" ? "Site" : "Produto"}{item.metadata?.size ? ` • ${Math.round(item.metadata.size/1024)} KB` : ""}</small><div><button onClick={() => void copy(item.url)}><Icon name="copy" />Copiar URL</button><button className="danger" onClick={() => void remove(item)}><Icon name="trash" /></button></div></article>)}</div>{!visible.length ? <div className="empty-ui"><Icon name="media" size={34} /><h3>Nenhuma imagem encontrada</h3></div> : null}</>;
}
