        ...
        <div className="clean-settings-layout">
          ...
          <div className="clean-settings-content">
            ...
            {tab === "home" ? <section className="admin-panel clean-settings-panel">
              <div className="admin-panel-head"><div><h2>Mensagem para quem veio da bio</h2><p>Deixe claro que os produtos mostrados nos seus vídeos estão logo no início do site.</p></div></div>
            ...
            {tab === "sections" ? <section className="admin-panel clean-settings-panel">
              ...
              <div className="clean-section-toggles">
                <label><input type="checkbox" checked={form.coverflow_enabled} onChange={(e) => update("coverflow_enabled", e.target.checked)} /><span><strong>Produtos dos vídeos passando</strong><small>Mostra primeiro os produtos marcados como “Produto do vídeo”.</small></span></label>
              ...
              <div className="admin-form-grid settings-fields"><label className="admin-label">Título da seção “Link da bio”<input className="admin-input" value={form.coverflow_title} onChange={(e) => update("coverflow_title", e.target.value)} /></label><label className="admin-label">Descrição<input className="admin-input" value={form.coverflow_subtitle} onChange={(e) => update("coverflow_subtitle", e.target.value)} /></label><label className="admin-label">Tempo entre produtos (ms)<input className="admin-input" type="number" min="2500" max="15000" step="500" value={form.carousel_speed} onChange={(e) => update("carousel_speed", Number(e.target.value))} /></label></div>
            </section> : null}
