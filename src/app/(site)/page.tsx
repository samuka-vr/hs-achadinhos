...
import { normalizeProducts, PUBLIC_PRODUCT_SELECT } from "@/lib/catalog";
...
const fallbackSections: HomeSection[] = [
  ...
  { id: "video", section_key: "video-products", section_type: "video_products", title: "Produtos dos últimos vídeos", subtitle: "Os links que acabaram de aparecer nas redes.", eyebrow: "DIRETO DO FEED", is_enabled: true, sort_order: 30, settings: { limit: 6, layout: "rail" }, created_at: "", updated_at: "" },
  { id: "newest", section_key: "newest", section_type: "newest", title: "Novidades no catálogo", subtitle: "Achados adicionados recentemente.", eyebrow: "ACABARAM DE CHEGAR", is_enabled: true, sort_order: 40, settings: { limit: 4, columns: 4 }, created_at: "", updated_at: "" },
  { id: "trending", section_key: "trending", section_type: "trending", title: "Mais procurados", subtitle: "Produtos que estão recebendo mais acessos.", eyebrow: "EM ALTA", is_enabled: true, sort_order: 50, settings: { limit: 4, columns: 4 }, created_at: "", updated_at: "" },
  { id: "catalog", section_key: "catalog", section_type: "catalog", title: "Mais achadinhos para você", subtitle: "Uma seleção rápida. O catálogo completo fica em uma página própria.", eyebrow: "CONTINUE EXPLORANDO", is_enabled: true, sort_order: 60, settings: { page_size: 4 }, created_at: "", updated_at: "" },
];
...
export default async function HomePage() {
  ...
  if (supabase) {
    const [productResult, categoryResult, settingsResult, sectionResult, bannerResult] = await Promise.all([
      supabase.from("products").select(PUBLIC_PRODUCT_SELECT).eq("is_active", true).order("is_pinned", { ascending: false }).order("created_at", { ascending: false }).limit(60),
    ...
    products = normalizeProducts(productResult.data);
  ...
  const heroProducts = [...videoProducts, ...newest.filter((item) => !videoProducts.some((video) => video.id === item.id))].filter((item) => Boolean(item.image_url?.trim())).slice(0, 12);
      ...
      {enabled.map((section) => {
        if (section.section_type === "hero") return <section className="hs-hero" key={section.id}>
          <div className="hs-hero__copy">
            ...
            <div className="hs-hero__quick-links"><a href="#produtos-dos-videos"><span><Icon name="sparkles" /></span><div><strong>Dos vídeos</strong><small>Links recentes</small></div></a><a href="#categorias"><span><Icon name="categories" /></span><div><strong>Categorias</strong><small>Escolha uma área</small></div></a><Link href="/catalogo"><span><Icon name="products" /></span><div><strong>Catálogo</strong><small>Ver tudo</small></div></Link></div>
        ...
        if (section.section_type === "video_products") return videoProducts.length ? <section className="hs-section hs-section--video" id="produtos-dos-videos" key={section.id}><SectionHeading section={section} action="/catalogo" />{String(section.settings.layout || "rail") === "grid" ? <ProductGrid products={videoProducts.slice(0, Math.min(safeNumber(section.settings.limit, 6), 6))} columns={safeNumber(section.settings.columns, 4)} /> : <ProductRail products={videoProducts.slice(0, Math.min(safeNumber(section.settings.limit, 6), 6))} autoplay={Boolean(section.settings.autoplay ?? true)} interval={safeNumber(section.settings.interval, settings.carousel_speed)} />}</section> : null;
        if (section.section_type === "newest") return newest.length ? <section className="hs-section" key={section.id}><SectionHeading section={section} id="novidades" action="/catalogo" /><ProductRail products={newest.slice(0, Math.min(safeNumber(section.settings.limit, 4), 4))} autoplay={false} /></section> : null;
        if (section.section_type === "trending") return trending.some((item) => item.click_count > 0) ? <section className="hs-section hs-section--tinted" key={section.id}><SectionHeading section={section} id="mais-acessados" action="/catalogo" /><ProductRail products={trending.filter((item) => item.click_count > 0).slice(0, Math.min(safeNumber(section.settings.limit, 4), 4))} autoplay={false} /></section> : null;
        if (section.section_type === "catalog") return <section className="hs-section hs-section--catalog" id="produtos" key={section.id}><SectionHeading section={section} action="/catalogo" /><ProductGrid products={newest.slice(0, 4)} columns={4} /><div className="hs-home-catalog__footer"><Link href="/catalogo">Ver catálogo completo <Icon name="arrow" size={17} /></Link><small>{products.length} produtos disponíveis</small></div></section>;
