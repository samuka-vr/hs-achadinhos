...
export default function AdminDashboardPage() {
  ...
  return (
    <>
      <div className="admin-page-heading"><div><span className="admin-eyebrow">VISÃO GERAL</span><h1>Olá! Veja como está seu catálogo.</h1><p>Acompanhe produtos, categorias e o interesse dos visitantes.</p></div><div className="admin-heading-actions"><Link className="admin-button secondary" href="/" target="_blank">Ver site <Icon name="external" /></Link><Link className="admin-button" href="/admin/produtos"><Icon name="plus" />Novo produto</Link></div></div>
      ...
      <div className="admin-stat-grid">
        ...
        <div className="admin-stat-card tone-purple"><span className="admin-stat-icon"><Icon name="categories" /></span><div><small>Categorias</small><strong>{categoryCount}</strong><em>organizando o catálogo</em></div></div>
      ...
      <div className="admin-dashboard-grid">
        <section className="admin-panel admin-chart-panel"><div className="admin-panel-head"><div><small>DESEMPENHO</small><h2>Cliques nos últimos 30 dias</h2></div><span className="admin-panel-badge"><Icon name="chart" size={16} />Atualizado</span></div><ClicksChart data={stats.points} /></section>
        <section className="admin-panel admin-quick-panel"><div className="admin-panel-head"><div><small>ATALHOS</small><h2>Ações rápidas</h2></div></div><div className="admin-quick-links"><Link href="/admin/produtos"><span><Icon name="plus" /></span><div><strong>Adicionar produto</strong><small>Cadastre um novo link da Shopee</small></div><Icon name="arrow" /></Link><Link href="/admin/categorias"><span><Icon name="categories" /></span><div><strong>Organizar categorias</strong><small>Edite ordem, imagem e status</small></div><Icon name="arrow" /></Link><Link href="/admin/configuracoes"><span><Icon name="settings" /></span><div><strong>Personalizar site</strong><small>Logo, cores, textos e redes sociais</small></div><Icon name="arrow" /></Link></div></section>
      </div>
